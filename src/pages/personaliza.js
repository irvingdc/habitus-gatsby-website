...
class Personaliza extends Component {
  ...
  selectOption = (field, value) => {
    ...
    switch (field) {
      case "usage":
        ...
        state = {
          usage: this.state.usage.map(it => ({
            ...it,
            selected: it.value === value
          })),
          materials: this.state.materials.map(it => ({
            ...it,
            selected: false,
            disabled: !allowedMaterials.find(val => val === it.value)
          })),
          finishStyles: this.state.finishStyles.map(it => ({
            ...it,
            selected: false,
            disabled: false
          })),
          thicknesses: this.state.thicknesses.map(it => ({
            ...it,
            selected: false
          }))
        };
      ...
      case "materials":
        state = {
          ...
          finishStyles: this.state.finishStyles.map(it => ({
            ...it,
            selected: false,
            disabled: !(value === "mdf" || value === "acero al carbón")
          })),
          thicknesses: this.state.thicknesses.map(it => ({
            ...it,
            selected: false
  ...
  filterThicknesses = () => {
    ...
    switch (materialValue) {
      ...
      case "acero inoxidable":
        options = ["0.91 mm (cal.20)", "1.21 mm (cal.18)", "1.51 mm (cal.16)"];
        break;
      case "aluminio":
        options = [
          "0.91 mm (cal.20)",
        ...
        break;
      case "mdf":
        options = ["18 mm", "15 mm", "12 mm", "9 mm", "5.5 mm", "3 mm"];
        break;
      case "triplay":
        options = ["18 mm", "12 mm", "5.5 mm"];
        break;
    }
    return this.state.thicknesses.filter(it =>
      options.find(option => it.value === option)
    );
  };
  ...
  sendForm = event => {
    ...
    if (this.state.height > 2440) {
      ...
      if (!this.state.materials.find(it => it.selected))
        errors.push("material");
      if (!this.state.thicknesses.find(it => it.selected))
        errors.push("espesor");
  ...
  render() {
    ...
    let bannerStyle =
      window.innerWidth < 770 ? { padding: "0px" } : { padding: "20px" };
    let headerStyle =
      window.innerWidth < 770
        ? { letterSpacing: "5px", fontSize: "28px", margin: "30px 10px" }
        : { letterSpacing: "20px", fontSize: "28px", margin: "30px" };
    ...
    return (
      <Fragment>
        ...
        <form className={classes.container} onSubmit={this.sendForm}>
          ...
          <DesignsCarousel selectable onChange={this.selectLattice} />
          ...
          <div className={classes.materialsSection}>
            {this.state.materials.map((it, index) => (
              <Option
                ...
                containerstyle={{
                  padding:
                    it.value === "aluminio" && window.innerWidth < 770
                      ? "10px 80px"
                      : "0px"
                }}
                value={it.value}
          ...
          <p className={classes.noteA}>
            * Solo aplica para MDF y Acero al carbón
          </p>
          ...
          <div className={classes.sizeSection}>
            <div>
              ...
              <p className={this.state.width > 1220 ? classes.error : ""}>
                * máximo <span>1220</span> mm (4')
              </p>
              <input
                ...
                type="number"
                onChange={event =>
                  this.handleInputChange("width", event.target.value)
                }
              />
            </div>
            <div>
              ...
              <p className={this.state.height > 2440 ? classes.error : ""}>
                * máximo <span>2440</span> mm (8')
              </p>
              <input
                ...
                type="number"
                onChange={event =>
                  this.handleInputChange("height", event.target.value)
                }
              />
            </div>
          </div>
          <div className={classes.amountSection}>
            <div>
              <i className={classes.line} />
              <h3 className={classes.subheader}>
                <b>CANTIDAD A SOLICITAR</b>
              </h3>
              <p className={classes.noteC} />
              <input
                required
                type="number"
                onChange={event =>
          ...
          <i className={classes.line} />
          <p className={classes.noteD}>
            obten la cotización de tu celosía y la asesoria de nuestros expertos
          </p>
          ...
          <div className={classes.informationSection}>
            <span>
              ...
              <input
                ...
                type="text"
                onChange={event =>
                  this.handleInputChange("comments", event.target.value)
                }
              />
            </span>
            <span>
